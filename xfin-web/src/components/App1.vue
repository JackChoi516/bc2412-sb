<template>
  ...
  <div class="container mx-auto pt-18 px-2">
    ...
    <table class="table-fixed cursor-pointer">
      <caption class="table-title font-bold text-gray-700 pb-2">
        Cryptocurrency Exchange - Top 100 (Market Cap)
      </caption>
      <!-- head -->
      <thead>
      ...
      <tbody class="divide-y">
        ...
        >
          ...
          <td class="hidden sm:table-cell">
            <p style="color: rgb(26, 137, 165)">
              {{ $filters.comma_separator(coin.total_volume) }}
            </p>
          </td>
          <td class="pr-10 hidden sm:table-cell">
            <p style="color: rgb(26, 137, 165)">
              ${{ $filters.comma_separator(coin.market_cap) }}
            </p>
          </td>
        </tr>
      </tbody>
...
export default {
  ...
  setup() {
    ...
    const retrieveCoins = async () => {
      try {
        const response = await axios.get("http://localhost:8105/ogmarket");
